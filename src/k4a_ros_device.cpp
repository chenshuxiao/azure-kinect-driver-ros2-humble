    ...
    imu_stream_end_of_file_(false)
{
  ...
  this->declare_parameter<bool>("required", true);
  ...
  this->declare_parameter<float>("body_tracking_smoothing_factor", 0.f);
  this->declare_parameter<bool>("rescale_ir_to_mono8", false);
  this->declare_parameter<float>("ir_mono8_scaling_factor", 1.f);
...
void K4AROSDevice::imuPublisherThread()
{
  ...
  while (running_ && rclcpp::ok())
  {
    ...
    else if (k4a_playback_handle_)
    {
      ...
      while (static_cast<long>(last_imu_time_usec_) <= last_capture_time_usec_ && !imu_stream_end_of_file_)
      {
        std::lock_guard<std::mutex> guard(k4a_playback_handle_mutex_);
